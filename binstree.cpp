...
// Each node has an Up link to make it easy to go up to the parent in the tree.
// It must be set as nodes are inserted.
// Some deletion cases require Up link to be updated.
//
#include <iostream>
#include "binstree.h"
...
void BST::INorderTraversal(Vertex* V)
{
	if (V != NULL)
	{
		...
		cout << "Element: " << V->Elem << " Height: " << V->Height << " Balance: " << V->Balance << endl;	//display V's element, height and balance and do endl; 
...
void BST::remove(Vertex* V, Vertex* P)
{
	...
	else if (V->Left != NULL && V->Right == NULL) //if V has just the left child so bypass V (case 2)
	{
		...
		if (V->Elem < P->Elem)				//You need if then else to determine Parent's left or right
		{
